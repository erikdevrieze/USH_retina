...
library(xlsx) #for writing (=saving) excel files. If not installed yet run install.packages("xlsx")
library(readr)
library(data.table)
...
###################################################################################################################
########## quant same for dataset + long sample  -> first 3 equals atlas precisely (as should be)##################
###################################################################################################################

#Read full data excel file. 
s1_4_sorted_gene_assignments <- fread("dataset 1_CCS3/isoquant/00_aln.sorted.read_assignments.tsv") 

#show colnames
colnames(s1_4_sorted_gene_assignments)
...
s1_4.df.split2 <- cSplit(s1_4.df.split1, "structural_cat", "=", direction = "wide")
...
#plot events per run and per gene   _ RUN1                                
...
df_1.3_struct_cat <- sample1 %>% 
  group_by(run_1, structural_cat_2)%>% 
  summarise(n = n())


#plot events per run and per gene   _ RUN2        
...
df_2.3_struct_cat <- sample2 %>% 
  group_by(run_1, structural_cat_2)%>% 
  summarise(n = n())

ggsave("structural_category_counts_sample2.pdf", width = 24, height = 24, units = "cm")

#plot events per run and per gene   _ RUN3        m64167e_210902_121011
...
#plot events per run and per gene   _ RUN4        
...
#####
sample1 %>% 
  dplyr::filter("intron_retention" %in% assignment_events)
